...
describe('Result', () => {
    describe('constructors', () => {
        ...
        it('unwrapOrElse', () => {
            ...
            expect(Err('message').unwrapOrElse(() => 'error returned')).toEqual('error returned')
        });
        it('unwrapErr', () => {
            expect(() => Ok(2).unwrapErr()).toThrow( '`Result.unwrapErr()` on a `Ok` value');
            expect(Err('message').unwrapErr()).toEqual('message')
        });

        it('expects', () => {
            expect(Ok(true).expect('Not true')).toEqual(true);
            expect(() => Err('message').expect('error')).toThrow(
              'error'
            );
        });
