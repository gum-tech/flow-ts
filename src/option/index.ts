...
const noneBuilder = <T>(): None<T> => ({
	...
	unwrap: (): never => {throw new  ReferenceError('`Option.unwrap()` on a `None` value')},
	expect: <A>(a: A): never => {throw `${a}`},
	unwrapOr: (a: T): T => a,
	flatten: (): None<T> => None, 
...
const someBuilder = <T>(t: T): Option<T> => ({
	...
	filter: (predicate: Predicate<T>): Option<T> => predicate(t) ? Some(t) : None,
	unwrap: (): T => t,
	expect: <A>(_: A): T => t,
	unwrapOr: (_: T): T => t,
...
export const Some = <T>(t?: T | undefined): Option<T> => typeof t === 'undefined' ? noneBuilder<T>() : someBuilder<T>(t as T);
...
export const match = <T, A, B>(t: Option<T>) => ({ some: onSome, none: onNone }: Match<T, A, B>): A | B => isNone(t) ?  onNone() : onSome(t.value);
